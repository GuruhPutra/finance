...
export default function Transactions() {
  ...
  return (
    <div className="animate-fade">
      ...
      {/* Summary row */}
      <div className="grid grid-3 mb-4" style={{ gridTemplateColumns: "1fr 1fr 1fr" }}>
        <div className="card" style={{ textAlign: "center" }}>
          <div className="text-secondary text-sm">Pemasukan</div>
          <div style={{ fontWeight: 800, fontSize: 18, color: "var(--green)", marginTop: 4 }}>{formatCurrency(totalIncome)}</div>
        </div>
        <div className="card" style={{ textAlign: "center" }}>
          <div className="text-secondary text-sm">Pengeluaran</div>
          <div style={{ fontWeight: 800, fontSize: 18, color: "var(--red)", marginTop: 4 }}>{formatCurrency(totalExpense)}</div>
        </div>
        <div className="card" style={{ textAlign: "center" }}>
          <div className="text-secondary text-sm">Selisih</div>
          <div style={{ fontWeight: 800, fontSize: 18, color: (totalIncome - totalExpense) >= 0 ? "var(--green)" : "var(--red)", marginTop: 4 }}>
      ...
      <div className="card mb-4">
        <div className="filter-bar">
          <div className="search-input-wrap" style={{ flex: 1, minWidth: 150 }}>
          ...
          <select className="form-select" style={{ width: 150 }} value={filterAccount} onChange={(e) => setFilterAccount(e.target.value)}>
            <option value="all">Semua Akun</option>
            {accounts.map((a) => <option key={a.id} value={a.id}>{a.icon} {a.name}</option>)}
          </select>
          <select className="form-select" style={{ width: 130 }} value={sortOrder} onChange={(e) => setSortOrder(e.target.value)}>
            <option value="desc">Terbaru</option>
            <option value="asc">Terlama</option>
          </select>
          {(search || filterType !== "all" || filterCategory !== "all" || filterAccount !== "all") && (
            ...
            <table className="data-table">
              ...
              <tbody>
                {filtered.map((tx) => {
                  ...
                  return (
                    <tr key={tx.id}>
                      <td style={{ color: "var(--text-secondary)", fontSize: 12 }}>{formatDate(tx.date)}</td>
                      <td>
                      ...
                      <td>
                        <span style={{ fontSize: 12, color: "var(--text-secondary)" }}>{acc?.icon} {acc?.name || "-"}</span>
                      </td>
                      ...
                      <td style={{ textAlign: "right", fontWeight: 700, color: tx.type === "income" ? "var(--green)" : "var(--red)", fontSize: 13 }}>
                        {tx.type === "income" ? "+" : "-"}{formatCurrency(tx.amount, true)}
                      </td>
                      <td>
                        <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}>
                          <button className="btn btn-ghost btn-icon btn-sm" title="Edit" onClick={() => openEdit(tx)}>✏️</button>
                          <button className="btn btn-danger btn-icon btn-sm" title="Hapus" onClick={() => setDeleteConfirm(tx.id)}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  );
        ...
        <div className="modal-overlay" onClick={() => setDeleteConfirm(null)}>
          <div className="modal" style={{ maxWidth: 380 }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-danger" onClick={() => confirmDelete(deleteConfirm)}>Hapus</button>
            </div>
          </div>
        </div>
      )}
