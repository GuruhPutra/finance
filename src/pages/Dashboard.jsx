...
const DOUGHNUT_OPTS = {
  ...
  plugins: {
    legend: {
      position: "right",
      labels: { color: "#8888aa", font: { size: 11 }, padding: 10, boxWidth: 10, borderRadius: 5 },
    },
    tooltip: {
      backgroundColor: "rgba(13,13,20,0.95)",
      borderColor: "rgba(255,255,255,0.08)", borderWidth: 1,
...
export default function Dashboard() {
  const { transactions, categories, accounts, accountBalances, totalBalance, loading } = useApp();
    ...
    <div className="animate-fade">
      ...
      <div className="page-header-row">
        <div>
          ...
          <p className="page-subtitle">Ringkasan keuangan Anda bulan ini</p>
        </div>
      ...
      <div className="grid grid-4 mb-4">
        <StatCard label="Total Saldo" value={formatCurrency(totalBalance, true)} icon="💼" color="var(--blue)" cls="stat-card-balance" />
        <StatCard label="Pemasukan Bulan Ini" value={formatCurrency(income, true)} icon="📈" color="var(--green)" cls="stat-card-income" />
        <StatCard label="Pengeluaran Bulan Ini" value={formatCurrency(expense, true)} icon="📉" color="var(--red)" cls="stat-card-expense" />
        <StatCard label="Tabungan Bulan Ini" value={formatCurrency(savings, true)} icon="💰" color="var(--accent)" sub={`${savingsRate}% dari pemasukan`} cls="stat-card-savings" />
      </div>

      {/* Charts — uses grid-chart class (1.6fr 1fr → 1fr on tablet) */}
      <div className="grid grid-chart mb-4">
        <div className="card">
      ...
      <div className="grid grid-2">
        <div className="card">
          <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 14 }}>Saldo Akun</div>
          {accounts.length === 0 ? (
              ...
              {accounts.map((acc) => (
                <div key={acc.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", background: "var(--bg-input)", borderRadius: "var(--radius-md)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: "var(--radius-sm)", background: (acc.color || "#6366f1") + "20", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>{acc.icon}</div>
                    <div>
                  ...
                  <div style={{ fontWeight: 700, fontSize: 13, color: (accountBalances[acc.id] || 0) >= 0 ? "var(--green)" : "var(--red)" }}>
        ...
        <div className="card">
          <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 14 }}>Transaksi Terbaru</div>
          {recent.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {recent.map((tx) => {
                ...
                return (
                  <div
                    key={tx.id}
                    style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 10px", borderRadius: "var(--radius-md)", transition: "background var(--transition)", cursor: "default" }}
                    onMouseEnter={(e) => e.currentTarget.style.background = "var(--bg-input)"}
                    onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                      <div style={{ width: 30, height: 30, borderRadius: "var(--radius-sm)", background: (cat?.color || "#6366f1") + "22", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, flexShrink: 0 }}>{cat?.icon || "💳"}</div>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontWeight: 600, fontSize: 12 }} className="truncate">{cat?.name || "Lainnya"}</div>
                        <div style={{ fontSize: 10, color: "var(--text-muted)" }}>{formatRelative(tx.date)}</div>
                      </div>
                    </div>
                    <div style={{ fontWeight: 700, fontSize: 13, color: tx.type === "income" ? "var(--green)" : "var(--red)", flexShrink: 0, marginLeft: 8 }}>
