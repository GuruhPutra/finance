  ...
  return (
    <div className="animate-fade">
      <div className="page-header">
        ...
        <p className="page-subtitle">Analisis perilaku keuangan dan rekomendasi cerdas berbasis data historis</p>
      </div>
      ...
      <div className="grid grid-4 mb-4">
        <div className="card text-center" style={{ background: `linear-gradient(135deg, ${savingsColor}11, ${savingsColor}05)`, borderColor: savingsColor + "33" }}>
          <div style={{ fontSize: 42, fontWeight: 900, color: savingsColor }}>{savingsRate}%</div>
        ...
        <div className="card text-center">
          <div style={{ fontSize: 42, fontWeight: 900, color: "var(--accent)" }}>{topCategories.length}</div>
          <div style={{ fontWeight: 600, fontSize: 13, marginTop: 4 }}>Kategori Aktif</div>
          <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>Kategori dengan pengeluaran</div>
        </div>
        <div className="card text-center">
          <div style={{ fontSize: 42, fontWeight: 900, color: "var(--blue)" }}>{transactions.length}</div>
        ...
        <div className="card text-center">
          <div style={{ fontSize: 22, fontWeight: 900, color: "var(--purple)", marginTop: 8 }}>
            {formatCurrency(topCategories[0]?.total || 0, true)}
          </div>
          <div style={{ fontWeight: 600, fontSize: 13, marginTop: 4 }}>Pengeluaran Terbesar</div>
          <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>{topCategories[0]?.label || "—"}</div>
        </div>
      </div>
      ...
      <div className="grid grid-2 mb-4">
        <div className="card">
          ...
          {topCategories.length === 0 ? (
            <div className="empty-state" style={{ padding: "40px 0" }}><div className="empty-state-icon">📊</div><div className="empty-state-desc">Belum ada data</div></div>
          ) : (
            <div style={{ height: 240 }}>
              <Bar data={barData} options={BASE_CHART_OPTS} />
            </div>
          )}
        </div>
        <div className="card">
          ...
          {trend.length === 0 ? (
            ...
            <div style={{ height: 224 }}>
              <Line data={lineData} options={{ ...BASE_CHART_OPTS, plugins: { ...BASE_CHART_OPTS.plugins, legend: { display: false } } }} />
            </div>
          )}
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {recommendations.map((tip, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "14px 16px", borderRadius: "var(--radius-md)", background: "var(--bg-input)", border: "1px solid var(--border)" }}>
              <span style={{ fontSize: 22, lineHeight: 1.2 }}>{tip.icon}</span>
              <span style={{ fontSize: 14, color: "var(--text-secondary)", lineHeight: 1.6 }}>{tip.text}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {topCategories.length > 0 && (
        <div className="card mt-4">
          <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 16 }}>Rincian Pengeluaran per Kategori</div>
              ...
              return (
                <div key={cat.categoryId}>
                  <div className="flex justify-between items-center" style={{ marginBottom: 5 }}>
                    ...
                    <span style={{ fontWeight: 700, fontSize: 13, color: cat.color }}>{formatCurrency(cat.total, true)}</span>
                  </div>
